...
int<-ggplot(abmodels, aes(y=terminal_dry_tissue_mass , x= est_tissue_dry_mass_i)) 
int +  geom_jitter(aes(col=diet)) + geom_smooth(aes(col=diet), method="lm", se=F)
#it looks like there could be an interaction when plotting

#lets use a type II ANCOVA, a main effects model with type II sum of squares which reflects the unique contribution of each variable after adjusting for all other variables. This can adjust the effect of starting weight and let us focus on diet effects.
...
# Histogram
hist(righting.tests$righting_time_2)

# Q-Q plot
qqnorm(righting.tests$righting_time_2)
qqline(righting.tests$righting_time_2)

#make a model 
rightingmodel <- aov(righting_time_2 ~ diet + righting_time_1 , data = righting.tests)



#hmogeneity of variances:
plot(fitted(rightingmodel), residuals(rightingmodel))
abline(h = 0, lty = 2)

# Interpret the ANCOVA results
summary(rightingmodel)
summary(rightingmodel, type = "II")

righting.aov.HSD1 <- HSD.test(rightingmodel , trt = 'diet') 
print(righting.aov.HSD1)
plot(righting.aov.HSD1)

## Perform pairwise comparison using t-tests
pairwise_results <- pairwise.t.test(bar$mean_time, bar$diet,
                                   paired = TRUE,
                                   p.adjust.method = "bonferroni")

# Extract the results
pairwise_comparison <- data.frame(pairwise_results$p.value)
rownames(pairwise_comparison) <- levels(bar$diet)
colnames(pairwise_comparison) <- c("righting_time_1 - righting_time_2")

# Display the pairwise comparison results
print(pairwise_comparison)


# no significance despite the visible pattern 
...
fig1<- ggplot(choice, aes(x=algae_spp., y= algae_Wd, fill = algae_spp.)) + geom_boxplot(show.legend = FALSE) + theme_bw()  + xlab("") + ylab("Seaweed Consumed (g)") + ggtitle("") + theme(axis.text=element_text(size=7),
...
fig2<- ggplot(performance, aes(x=diet, y= terminal_dry_tissue_mass, fill= diet)) + 
  ...
  facet_grid(grazer_spp. ~ ., scales = "free_y", labeller = as_labeller(c("red abalone" = "Red abalone", "tegula" = "Turban snails"))) +  theme(strip.text = element_text()) +
...
rightingfig <- ggplot(bar, aes(x = diet, y = mean_time, fill = trial)) +
  ...
  scale_x_discrete(limits = c("starved", "macro", "mixed", "sargassum"),
                   labels = c("starved" = "Starved", "macro" = "Kelp",
                              "mixed" = "Mixed", "sargassum" = "Devilweed")) +
  geom_vline(xintercept = c(1.5), linetype = "dotted") +
  scale_fill_manual("", values = c("righting_time_1" = "#1260AB", "righting_time_2" = "#009BFF"),
...
commplot <- ggplot(community, aes(x = Algae_trt, y = Algae_Wd, fill = Wrack_treatment)) +
  xlab("") +
  ...
  scale_x_discrete(limits = c("Wrack", "Centroceras", "Ulva", "Silvetia"),
                   labels = c("Wrack" = "Wrack", "Centroceras" = "C. clavulatum", "Ulva" = "Ulva spp.", "Silvetia" = "S. compressa")) +
  geom_vline(xintercept = c(1.5), linetype = "dotted") +
  labs(fill = "Wrack Treatment") +
  scale_fill_manual(values = c("Invasive" = "orange3", "Native" = "olivedrab3")) +
