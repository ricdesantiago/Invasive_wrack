...
ggplot(performance, aes(y=percent_change, x=diet)) + geom_boxplot()
...
print(teg.aov.HSD1)
plot(teg.aov.HSD1)
#S.horneri and mixed diets are not different from each other, M.pyrifera resulted ina  lower mean
```
...
#We can compare the means of total native benthic algae using a T test to determine if there is a statistical difference


```

##Community Assay Analysis
```{r}
#lets check the normality of the data for a T test
