...
tegbox<-ggplot(tegulachoice, aes(x=algae_spp., y=algae_Wd)) + geom_boxplot()
tegbox
...
comm<-subset(community, Treatment=="Exp")
view(comm)
```
...
shapiro_test(comm_ulva$Algae_Wd)
comm_sil$Algae_Wd <- log(comm_ulva$Algae_Wd)
...
shapiro_test(comm_sil$Algae_Wd)
comm_sil$Algae_Wd <- log(comm_sil$Algae_Wd)
...
shapiro_test(comm_cen$Algae_Wd)
#log transform
comm_cen$Algae_Wd <- log(comm_cen$Algae_Wd)
...
#histograms for data with two factors
...
#boxplot
boxplot(Algae_Wd ~ Wrack_treatment * Algae_Spp., data = NBS_Sub2, xlab = "Wrack_treatment.Algae_Spp.", ylab = "Algae_Wd")
...
#practice
commmodel<-lme(Algae_Wd~Algae_Spp.+ Wrack_treatment, random = ~1 | Rep_No., data = NBS_Sub2)
summary(commmodel)
anova(commmodel)
coef(commmodel)
plot(ranef(commmodel))
plot(commmodel)

NBS_Sub2$Wrack_treatment <- factor(NBS_Sub2$Wrack_treatment, levels = c("Ulva", "Centroceras", "Silvetia"))
NBS_Sub2$Wrack_treatment <- as.factor(NBS_Sub2$Wrack_treatment)

tukey <- glht(commmodel, linfct = mcp(Wrack_treatment = "Tukey"))
##need to get this to work fasf
...
### If we want to understand how consumption changes by species, we can can change S. horneri and M. pyrifera to "wrack" and compare all seaweed species across Wrack treatment (Eg, Ulva, Silvetia, Centroceras, and Wrack by M and S). We should  expect to see a decrease in "wrack" and an increase in "silvetia."

## lets look at this using ANOVA
#H0: There is no difference in  seaweed consumption between Invasive (S) and Native (M) treatments
#HA: There is a difference in  seaweed consumption between Invasive (S) and Native (M) treatments
aov2<- aov(Algae_Wd ~ Algae_trt, data =community)
summary(aov2)

### We reject the null hypothesis and embrace the alternative, there is a difference in consumption of seaweeds

### now lets look how consumption differs using a post-hoc test 
NBSposthoc <- HSD.test(aov2 , trt = 'Algae_trt') 
print(NBSposthoc)
plot(NBSposthoc)

# Silvetia    3.3795300      a
# Wrack       1.2604175      b
# Centroceras 0.4801150      c
# Ulva        0.4767825      c
```
