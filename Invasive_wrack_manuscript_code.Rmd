...
scatter<-ggplot(community, aes(y=Algae_Wd , x= Algae_Spp.)) +  geom_jitter(aes(col=Wrack_treatment)) + geom_smooth(aes(col=Algae_Spp.), method="lm", se=F)
#In general, we see that consumption tends to be clustered equally for Ulva and Centroceras but there is a vertical shift in Silvetia consumption when going from M to S.

#We can nest NBS treatments within Wrack treatment to ask
###########################"Does consumption of NBS species differ within Wrack treatments?##################
#first, remove Macro and Sargassum from the population 
commedited<-subset(community, !Algae_Spp.== "Sargassum")
view(commedited)
commedited2<-subset(commedited, !Algae_Spp.== "Macro")
view(commedited2)
...
NestedNBSaov<-aov(Algae_Wd ~  Algae_Spp. + Wrack_treatment + Error(Wrack_treatment:Algae_Spp.), data =commedited2)
summary(NestedNBSaov)
##we reject the H0  (p= 0.0127) and embrace the alternative, consumption is not the same between algae species. 
##We fail to reject H1 (p= 0.2904), consumption is the same between wrack treatments 

#we can quickly visualize that here
#H0
ggplot(commedited2, aes(x = Wrack_treatment, y=Algae_Wd)) + geom_boxplot()
#H1
ggplot(commedited2, aes(x = Wrack_treatment, y=Algae_Wd)) + geom_boxplot()
...
#Therefore, if we want to understand how consumption changes by species, we need to consider S. horneri and M. pyrifera as "wrack" and compare all Algae species across Wrack treatment. We should can expect to see a decrease in "wrack" and an increase in "silvetia."

###So
completeaov<-lm(Algae_Wd ~ Algae_Spp. + Wrack_treatment, data =community)
anova(completeaov)
summary(completeaov)
