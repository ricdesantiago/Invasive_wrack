...
###So
aov2<- aov(Algae_Wd ~ Wrack_treatment * Algae_trt, data =community)
anova(aov2)
summary(aov2)

#Here we see no difference when looking at consumption by wrack treatment 
#significant difference in algae treatment
#significant interaction between algae treatment and wrack treatment....

#the story here is that overall grazing does not differ between a "native wrack" vs "invasive wrack" treatments when we swap the wrack species because the grazing from S. horneri shifts to Silvetia... thats the story I am just not sure how to best show it. I can use some help!
...
#make bargraph
fig3<-ggplot(gathered_obs, aes(x=diet, y=time, fill= righting)) + xlab("") + ylab("Time (sec)") + ggtitle("") + theme(axis.text=element_text(size=12)) + geom_bar(stat="identity",position=position_dodge()) + theme_minimal() +  
  scale_x_discrete(limits = c("starved", "macro", "mixed", "sargassum"), labels=c("starved" = "Starved", "macro" = "M. pyrifera","mixed" = "Mixed","sargassum" = "S. horneri")) +  theme(axis.text.x = element_text(face = "italic")) +
