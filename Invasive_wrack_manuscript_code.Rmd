...
ancova_model <- aov(terminal_dry_tissue_mass ~ diet + est_total_dry_mass_i, data = abmodels)
...
ggplot(tegulaperformance, aes(x=diet, y=terminal_dry_tissue_mass, fill= treatment)) + geom_boxplot() + theme_bw()   + xlab("Diet") + ylab("Tissue Dry Mass (g)") + ggtitle("Tegula final dry mass by diet") + theme(plot.title =element_text(hjust = 0.5))
...
#check normality
qqnorm(performance2$terminal_dry_tissue_mass)
plot(performance2$terminal_dry_tissue_mass)

#look at them individually
redabs<-subset(performance2, grazer_spp.== "red abalone")
hist(redabs$terminal_dry_tissue_mass)
tegs<-subset(performance2, grazer_spp.== "tegula")
hist(tegs$terminal_dry_tissue_mass)


#create simple model 
m <- lm(terminal_dry_tissue_mass ~ diet, performance2)
summary(m)
par(mfrow = c(2, 2))
plot(m)
anova(m)

m2<-lm(terminal_dry_tissue_mass ~ diet*grazer_spp., performance2)
summary(m2)
par(mfrow = c(2, 2))
plot(m2)
anova(m2)
...
m3<-glmer(terminal_dry_tissue_mass ~ diet*grazer_spp. + (1| est_total_dry_mass_i) , family = gaussian, performance2)
summary(m3)
par(mfrow = c(2, 2))
plot(m3)
anova(m3)
...
m3<- glmer(terminal_dry_tissue_mass ~ diet*grazer_spp. + (1| est_total_dry_mass_i), data = performance2, family = binomial)
...
model2<- aov(terminal_dry_tissue_mass ~ diet + est_total_dry_mass_i, data = tegs)
...
test<- aov(terminal_dry_tissue_mass ~ diet, data = redabs)
Anova(test)
...
model1<- aov(terminal_dry_tissue_mass ~ diet + est_total_dry_mass_i, data = redabs)
