...
int<-ggxplot(abmodels, aes(y=terminal_total_dry_mass , x= est_total_dry_mass_i)) 
...
abmodel <- aov(terminal_total_dry_mass ~ est_total_dry_mass_i * diet , data = abmodels)
summary(abmodel)
...
tegmodel <- aov(terminal_total_dry_mass ~ est_total_dry_mass_i * diet , data = tegmodels)
summary(tegmodel)
...
view(commSH)
ggplot(commSH, aes(x=Total_Benthic_Algae_Consumed)) + geom_histogram(binwidth=0.1)
...
#test if there is a difference in consumption between Native Benthic Seaweeds between S. hornery and M. pyrifera treatments
#H0: There is no difference in consumption on NBS between S and M treatments
#HA:There is a difference in consumption on NBS between S and M treatments
t.test(community$Total_Benthic_Algae_Consumed~community$Wrack_treatment, alt="two.sided", paired=F,conf.level=0.95)
##We fail to reject the null hypotheis (p-value = 0.0697) there is no difference in consumption of NBS when pooled.

##While not statistically significant (p-value = 0.0697), we can see consumption of NBS tends to be higher in the S treatment. We can look to see if that difference is driven by any one specific speces by testing the contribution of each species. To do this, we will conduct an Analysis of variance between individual NBS by wrack species

NBSaov<-lm(Algae_Wd ~ Wrack_treatment, data =community)

par(mfrow=c(2,2))
ggqqplot(residuals(NBSaov))
