...
commmodel3<-lme(Algae_Wd~ Wrack_treatment * Algae_Spp., random = ~1 | Rep_No., data = NBS_Sub)
summary(commmodel3) 
# make model to see if interaction factor is necessary
commmodel4<-lme(Algae_Wd~ Wrack_treatment + Algae_Spp., random = ~1 | Rep_No., data = NBS_Sub)
summary(commmodel4) 
#we reject the null hypothesis and accept the alternative: Consumption differs between wrack treatment

#goodness of fit

anova(commmodel3,commmodel4)
...
p1 <- ggplot(community, aes(x= Wrack_treatment , y= Total_Algae_Consumed)) + theme_bw() + geom_boxplot() + ggtitle("") + ylab("Total Consumed (g)") + xlab("Wrack treatment") + theme(plot.title =element_text(hjust = 0.5))

p2 <- ggplot(community, aes(x= Wrack_treatment , y= Total_Benthic_Algae_Consumed )) + theme_bw() + geom_boxplot() + ggtitle("") + ylab("Total Consumption(g)") + xlab("Wrack treatment") + theme(plot.title =element_text(hjust = 0.5))
...
rightingfig <- ggplot(bar, aes(x=diet, y=mean_time, fill= trial)) +
  ...
  geom_errorbar(aes(x=diet,ymin=mean_time-se, ymax=mean_time+se), width=.2, position=position_dodge(.9))
...
#import and name data
agar <-read.csv("LabAssaysAgar.csv",header=T)
View(agar)

#ensure factors are correctly read
is.numeric(agar$Algae_Wd)
is.factor(agar$Algae_Spp.)
agar$Algae_Spp.<-as.factor(agar$Algae_Spp.)

agarassays<-ggplot(agar, aes(x=Grazer_Spp., y=Algae_Wd, fill= Algae_Spp.)) + geom_boxplot() + theme_bw()   + xlab("Algae Species") + ylab("Total Consumed (g)") + ggtitle("Live algae choice assay") + theme(plot.title =element_text(hjust = 0.5))
agarassays
```
